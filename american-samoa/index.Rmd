---
title: "Explore OHI Global Data & Data Gaps for American Samoa"
output:
  html_document:
    toc: true
    toc_float: true
...
To explore the OHI Global data for American Samoa, focusing on where data are estimated (gapfilled) instead of available in global data sets for American Samoa.

----

## Gapfilling in American Samoa
...
- **catch by species**: only what is fished within American Samoa's EEZ
- **years**: at least 10 years of data (preferably 20)


We can look at these data in several ways. Let's start by looking on SAUP's website at an interactive visualization of fish caught in American Samoa's exclusive economic zone (EEZ). Here's how: 

1. Navigate to [seaaroundus.org](http://www.seaaroundus.org)
1. Click "Tools & Data"
1. Search "American Samoa"

We can also look at the mapped locations of fishing effort by American Samoa (remember that OHI only includes catch caught withing American Samoa's EEZ. 

1. Click "Mapped Data"

<br>

Let's look through the species listed and see if there are any included that don't represent American Samoa's fisheries within the EEZ. 
...
There are `r fis_meancatch_lookup %>% nrow()` species in the [fis_meancatch](https://github.com/OHI-Science/<%=key%>/blob/master/global_explore/data/fis_meancatch_lookup.csv) layer for American Samoa.
...
There are only `r fis_b_bmsy_lookup %>% nrow()` species in the [fis_b_bmsy](https://github.com/OHI-Science/<%=key%>/blob/master/global_explore/data/fis_b_bmsy_lookup.csv) layer for American Samoa.
...
- are there other species that should be represented in American Samoa?
...
Let's have a look at the data included for American Samoa:
...
Which Natural Products are included for American Samoa?
...
- is this a goal that represents American Samoa well?
...
#             by = "sciname") %>%
#   select(sciname, comname, iucn_sid, cat_code) # n_spp_rgn; this = n_rows: 1467 for American Samoa
# 
